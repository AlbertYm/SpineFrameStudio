...
## 2026.9.23 无 FFmpeg 公开包

已采用上文方案 3：`tools/build.ps1 -ForPublicRelease -WithoutFfmpeg` 生成不含 `ffmpeg.exe` 的公开应用包。构建脚本仍会阻止在 `redistributionReady=false` 时把旧 FFmpeg 放入公开包。无 FFmpeg 包包含应用 EXE、全部项目脚本、许可和来源档案，并提供 `FFMPEG_REQUIRED_先看这里.txt`，要求用户从可信来源独立配置 FFmpeg。此路径不改变旧 FFmpeg 的审计结论，也不把无 FFmpeg 包描述为离线全依赖便携包。
